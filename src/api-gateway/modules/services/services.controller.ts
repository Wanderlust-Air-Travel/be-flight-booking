...
export class ServicesController {
	...
	async getDeals(): Promise<GetDealsResponseDto> {
		try {
			return await firstValueFrom(this.client.send<GetDealsResponseDto>('services.get-deals', {}));
		} catch (error: any) {
			...
			if (error?.code === 'ECONNREFUSED' || error?.message?.includes('ECONNREFUSED')) {
				throw new Error('Services microservice is not running. Please start it with: npm run start:services');
			}
			if (error?.code === 'ETIMEDOUT' || error?.message?.includes('timeout')) {
				throw new Error('Services microservice request timeout. Please check if the service is running.');
			}
