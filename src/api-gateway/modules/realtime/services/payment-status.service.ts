...
import { PaymentStatusMessage } from '../types/payment-status.types';
...
export class PaymentStatusService implements OnModuleInit, OnModuleDestroy {
	...
	async unsubscribe(socketId: string, bookingId: string): Promise<void> {
		...
		const subscribedClients = this.realtimeService.getSubscribedClients('paymentStatus', bookingId);
	...
	async publishPaymentStatusChange(
		...
		this.logger.debug(`Published payment status change: booking=${bookingId}, payment=${paymentId}, status=${status}`);
	}
