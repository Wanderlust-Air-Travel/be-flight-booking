...
export class MockPaymentGateway implements IPaymentGateway {
	...
	async createPayment(payment: Payment, booking: Booking): Promise<PaymentGatewayResponse> {
		this.logger.log(`[MOCK] Creating payment ${payment.payment_id} for booking ${booking.booking_id}`);
		...
		return {
			...
			paymentUrl,
			status: 'pending',
			message: 'Payment created successfully. Redirect user to paymentUrl to complete payment.',
		};
	}
	...
	async processRefund(transactionId: string, amount: number): Promise<string> {
		this.logger.log(`[MOCK] Processing refund for transaction ${transactionId}, amount: ${amount}`);
