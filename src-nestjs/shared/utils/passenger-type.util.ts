...
import type { PassengerValidationResult } from '../types/passenger-type.types';
...
export function validatePassengerTypes(
    ...
    if (infants.length > 0) {
        ...
        if (infants.length > adults.length) {
            errors.push(
                `Each adult can only accompany maximum 1 infant. You have ${adults.length} adult(s) but ${infants.length} infant(s). Additional infant(s) must be booked as child (CHD).`
            );
        }
    }
    ...
    for (const passenger of passengers) {
        ...
        if (passenger.passengerType !== actualType) {
            ...
            if (passenger.passengerType === PassengerType.INF && actualType === PassengerType.CHD) {
                errors.push(
                    `Passenger ${passenger.dob.toISOString()} is 2 years or older and should be booked as CHD, not INF`
                );
            } else if (passenger.passengerType !== actualType) {
                errors.push(
                    `Passenger type mismatch: Expected ${actualType} but got ${passenger.passengerType} for age ${calculateAge(passenger.dob, flightDate)}`
                );
            }
        }
    }
