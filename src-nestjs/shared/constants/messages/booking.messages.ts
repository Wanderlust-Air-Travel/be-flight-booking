...
export const BOOKING_MESSAGES = {
    ...
    VALIDATION: {
        ...
        CONTACT_INFO_REQUIRED_FOR_GUEST:
            'Thông tin liên hệ (họ tên, email, số điện thoại) là bắt buộc cho đặt vé khách.',
        TICKET_ID_INVALID_FORMAT: 'Định dạng Ticket ID không hợp lệ. Yêu cầu UUID v7.',
        OTP_VERIFICATION_REQUIRED_PAID_BOOKING:
            'Xác thực OTP là bắt buộc để hủy đặt vé đã thanh toán. Vui lòng xác thực OTP trước bằng POST /api/v1/auth/otp/cancellation/verify',
        OTP_VERIFICATION_REQUIRED_PAID_TICKET:
            'Xác thực OTP là bắt buộc để hủy vé từ đặt vé đã thanh toán. Vui lòng xác thực OTP trước bằng POST /api/v1/auth/otp/cancellation/verify',
        INFANT_REQUIRES_ADULT:
            'Em bé (INF) phải được đi kèm bởi ít nhất một người lớn (ADT) từ 18 tuổi trở lên',
        TOO_MANY_INFANTS_PER_ADULT: (adults: number, infants: number) =>
            `Mỗi người lớn chỉ được đi kèm tối đa 1 em bé. Bạn có ${adults} người lớn nhưng ${infants} em bé. Em bé thứ hai trở đi phải đặt vé trẻ em (CHD)`,
        INFANT_CANNOT_HAVE_SEAT:
            'Em bé (INF) không được cung cấp ghế ngồi riêng. Em bé phải ngồi trong lòng người lớn đi kèm',
        ADULT_NOT_OLD_ENOUGH: 'Người lớn (ADT) phải từ 18 tuổi trở lên để có thể đi kèm em bé',
        PASSENGER_TYPE_MISMATCH: (expected: string, actual: string, age: number) =>
            `Loại hành khách không khớp: Tuổi ${age} yêu cầu loại ${expected} nhưng được chỉ định là ${actual}`,
    },
} as const;
