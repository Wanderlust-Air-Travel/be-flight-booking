...
import { DataProvider, RateLimitInfo } from '../enums/data-provider.enum';
import { FlightProvider, FlightStatusDto } from '../interfaces/flight-provider.interface';
import {
  AirportDto,
  AirlineDto,
  AircraftDto,
...
export class OurairportsProvider implements FlightProvider {
  ...
  async getAirports(): Promise<AirportDto[]> {
    ...
    try {
      const airportsCsv = await this.httpClient.get<string>(
        `${this.baseUrl}/airports.csv`,
        { timeout: 30000 },
      );
  ...
  async searchFlights(_params: FlightSearchParams): Promise<FlightSearchResultDto> {
    this.logger.warn('OurAirports provider does not support flight search. Use for reference data only.');
    return {
      flights: [],
      currency: 'USD',
      totalResults: 0,
  ...
  private parseAirportsCsv(csv: string): AirportDto[] {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      airports.push({
        ...
        timezone: 'UTC',
        latitude: parseFloat(values[4]) || undefined,
        longitude: parseFloat(values[5]) || undefined,
      });
    }
  ...
  private getCountryName(isoCode: string): string {
    const countryMap: Record<string, string> = {
      ...
      IN: 'India', AE: 'United Arab Emirates', QA: 'Qatar', SA: 'Saudi Arabia',
      HK: 'Hong Kong', TW: 'Taiwan', MM: 'Myanmar', KH: 'Cambodia', LA: 'Laos',
    };
  ...
  private getDefaultAirports(): AirportDto[] {
    return [
      { iata_code: 'SGN', name: 'Tan Son Nhat International Airport', city: 'Ho Chi Minh City', country: 'Vietnam', timezone: 'Asia/Ho_Chi_Minh' },
      { iata_code: 'HAN', name: 'Noi Bai International Airport', city: 'Hanoi', country: 'Vietnam', timezone: 'Asia/Ho_Chi_Minh' },
      { iata_code: 'DAD', name: 'Da Nang International Airport', city: 'Da Nang', country: 'Vietnam', timezone: 'Asia/Ho_Chi_Minh' },
      { iata_code: 'NRT', name: 'Narita International Airport', city: 'Tokyo', country: 'Japan', timezone: 'Asia/Tokyo' },
      { iata_code: 'HND', name: 'Haneda Airport', city: 'Tokyo', country: 'Japan', timezone: 'Asia/Tokyo' },
      { iata_code: 'ICN', name: 'Incheon International Airport', city: 'Seoul', country: 'South Korea', timezone: 'Asia/Seoul' },
      { iata_code: 'SIN', name: 'Singapore Changi Airport', city: 'Singapore', country: 'Singapore', timezone: 'Asia/Singapore' },
      { iata_code: 'BKK', name: 'Suvarnabhumi Airport', city: 'Bangkok', country: 'Thailand', timezone: 'Asia/Bangkok' },
      { iata_code: 'KUL', name: 'Kuala Lumpur International Airport', city: 'Kuala Lumpur', country: 'Malaysia', timezone: 'Asia/Kuala_Lumpur' },
      { iata_code: 'LAX', name: 'Los Angeles International Airport', city: 'Los Angeles', country: 'United States', timezone: 'America/Los_Angeles' },
      { iata_code: 'JFK', name: 'John F. Kennedy International Airport', city: 'New York', country: 'United States', timezone: 'America/New_York' },
      { iata_code: 'LHR', name: 'London Heathrow Airport', city: 'London', country: 'United Kingdom', timezone: 'Europe/London' },
      { iata_code: 'CDG', name: 'Charles de Gaulle Airport', city: 'Paris', country: 'France', timezone: 'Europe/Paris' },
      { iata_code: 'DXB', name: 'Dubai International Airport', city: 'Dubai', country: 'United Arab Emirates', timezone: 'Asia/Dubai' },
    ];
  }
}
