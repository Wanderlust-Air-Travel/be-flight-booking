...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        const prefetchCountRaw = this.configService.get<string | number>(
            'RABBITMQ_PREFETCH_COUNT',
            '10'
        );
        let prefetchCount: number;
        if (typeof prefetchCountRaw === 'number') {
            prefetchCount = Number.isNaN(prefetchCountRaw) ? 10 : prefetchCountRaw;
        } else {
            const parsed = Number.parseInt(prefetchCountRaw, 10);
            prefetchCount = Number.isNaN(parsed) ? 10 : parsed;
        }
     ...
     */
    async getChannel(channelName = 'default'): Promise<Channel> {
        if (!this.connection) {
            await this.connect();
        }
        ...
        if (Number.isNaN(prefetchCount) || prefetchCount <= 0) {
            this.logger.warn(
                `Invalid prefetchCount: ${this.config.prefetchCount}, using default: 10`
            );
                    ...
                    if (retryCount < maxRetries) {
                        ...
                        setTimeout(async () => {
                            try {
                                ...
                                if (!sent) {
                                    this.logger.error(
                                        'Failed to republish message after retry: sendToQueue returned false'
                                    );
                                }
                            } catch (err: any) {
                                this.logger.error(
                                    `Failed to republish message after retry: ${err.message}`
                                );
                            }
                        }, delay);
    ...
    async assertQueueWithDLQ(
        ...
        try {
            ...
            if (
                ...
                error.message?.includes('inequivalent')
            ) {
                this.logger.warn(
                ...
                try {
                    ...
                    if (process.env.NODE_ENV !== 'production') {
                        ...
                        } catch (_channelError: any) {
                            // Channel is closed, create a new one
                            this.logger.warn(
                                'Channel closed, creating new channel for queue deletion'
                            );
                        ...
                        this.logger.log(
                            `Deleted existing queue ${queue} to recreate with new config`
                        );
                        ...
                        const assertChannel =
                            deleteChannel !== ch
                                ? deleteChannel
                                : await this.getChannel('queue-management');
     ...
     */
    async bindQueue(queue: string, exchange: string, routingKey = ''): Promise<Replies.Empty> {
        const channel = await this.getChannel('default');
