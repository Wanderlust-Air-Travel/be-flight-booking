...
import { FareClass } from '../entities/fare/fare-class.entity';
import { CabinClass } from '../entities/cabin/cabin-class.entity';
...
export class CabinServiceService {
	...
	constructor(
		...
		private readonly baggageAllowanceRepo: Repository<BaggageAllowance>,
		@InjectRepository(CabinService)
		private readonly cabinServiceRepo: Repository<CabinService>,
	) {}
	...
	async getCabinServices(
		...
		try {
			...
			return Array.from(serviceMap.values()).sort((a, b) => a.display_order - b.display_order);
		} catch (error) {
			this.logger.error(
