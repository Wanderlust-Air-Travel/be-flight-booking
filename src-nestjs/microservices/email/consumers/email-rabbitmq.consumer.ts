...
export class EmailRabbitMQConsumer implements OnModuleInit, OnModuleDestroy {
	...
	private async startConsuming(): Promise<void> {
		try {
			...
			this.consumerTag = await this.rabbitMQService.consume(
				...
				async (msg) => {
					...
					try {
						...
						const { correlationId: _, messageId: __, timestamp: ___, idempotencyKey: ____, ...emailDto } =
							messageContent;
			...
			this.logger.log(`Started consuming emails from queue: ${this.queueName} (with DLQ support)`);
		} catch (error: any) {
