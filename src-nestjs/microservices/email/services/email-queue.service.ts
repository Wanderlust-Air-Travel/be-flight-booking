...
export class EmailQueueService implements OnModuleInit, OnModuleDestroy {
		...
		try {
			...
			if (email.retryCount >= email.maxRetries) {
				...
				const delay = Math.min(
					this.INITIAL_RETRY_DELAY_MS * Math.pow(2, email.retryCount - 1),
				...
				this.logger.warn(`Email ${email.id} will retry in ${delay}ms (attempt ${email.retryCount}/${email.maxRetries})`);
			}
		}
	}
	...
	private clearExpiredRateLimitEntries(): void {
		...
		while (this.emailsSentInWindow.length > 0 && this.emailsSentInWindow[0].getTime() < windowStart) {
