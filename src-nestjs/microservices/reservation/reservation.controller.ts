...
export class ReservationMsController {
    ...
    async handleCreateReservation(payload: {
        ...
        try {
            ...
            this.logger.log(
                `Reservation created: ${result.reservationId} (Code: ${result.reservationCode})`
            );
            return result;
        } catch (error: any) {
    ...
    async handleExtendReservation(payload: {
        ...
        try {
            this.logger.log(
                `Extend reservation: ${payload.reservationId}, additionalSeconds: ${payload.additionalSeconds}`
            );
            const result = await this.reservationService.extendReservation(
