...
export class TicketRabbitMQConsumer implements OnModuleInit, OnModuleDestroy {
    ...
    private async startConsuming(): Promise<void> {
        try {
            ...
            this.consumerTag = await this.rabbitMQService.consume(
                ...
                async (msg) => {
                    ...
                    try {
                        ...
                        const { timestamp, ...payload } = messageContent;
                        ...
                        this.logger.error(
                            `Error processing ticket creation message: ${error.message}`,
                            error.stack
                        );
            ...
            this.logger.log(
                `Started consuming ticket creation messages from queue: ${this.queueName} (with DLQ support)`
            );
        } catch (error: any) {
            this.logger.error(
                `Failed to start consuming ticket creation messages: ${error.message}`,
                error.stack
            );
        }
    }
