...
export class BookingMsController {
    ...
    // For guest bookings, userId will be null
    async handleCreateBookingFromReservation(payload: {
        reservationId: string;
        userId: string | null; // Receive userId (extracted by Gateway), or null for guest bookings
        ...
        try {
            ...
            this.logger.log(
                `Booking created from reservation: ${result.bookingId} (PNR: ${result.pnrCode})`
            );
            return result;
        } catch (error: any) {
    ...
    async handleGetBooking(payload: {
        bookingId: string;
        userId: string | null;
    }): Promise<GetBookingResponseDto> {
        try {
            this.logger.log(
                `Get booking: ${payload.bookingId}, userId: ${payload.userId || 'guest'}`
            );
    ...
    async handleUpdateBookingPassengers(payload: {
        ...
        try {
            ...
            const result = await this.bookingService.updateBookingPassengers(
                payload.bookingId,
                payload.dto
            );
            return result;
    ...
    async handleGetMyTickets(payload: {
        userId: string;
        dto: GetMyTicketsDto;
    }): Promise<MyTicketsResponseDto> {
        try {
            this.logger.log(
                `Get my tickets: userId=${payload.userId}, page=${payload.dto?.page}, limit=${payload.dto?.limit}`
            );
    ...
    async handleCreateTicketsFromBooking(payload: { bookingId: string }): Promise<{
        success: boolean;
        ticketCount: number;
    }> {
        try {
            ...
            this.logger.log(
                `Successfully created ${tickets.length} tickets for booking ${payload.bookingId}`
            );
    ...
    async handleCancelBooking(payload: { bookingId: string; userId: string }): Promise<{
        success: boolean;
        message: string;
        refundAmount?: number;
    ...
    async handleCancelTicket(payload: { ticketId: string; userId: string }): Promise<{
        success: boolean;
        message: string;
        refundAmount?: number;
        cancellationFee?: number;
        ...
        try {
            ...
            this.logger.log(
                `Ticket ${payload.ticketId} cancelled successfully${result.bookingCancelled ? ', booking auto-cancelled' : ''}`
            );
            return result;
        } catch (error: any) {
    ...
    async handleGetTicketInfo(payload: { ticketId: string; userId: string }): Promise<{
        ticketId: string;
        bookingId: string;
        ...
        try {
            ...
            const result = await this.bookingService.getTicketInfo(
                payload.ticketId,
                payload.userId
            );
            return result;
    ...
    async handleCheckInBooking(dto: CheckInBookingDto): Promise<CheckInBookingResponseDto> {
        try {
            ...
            this.logger.log(
                `Check-in completed for booking ${dto.bookingCode}, created ${result.ticketCount} tickets`
            );
            return result;
        } catch (error: any) {
