...
import type { Ticket } from 'src/shared/entities/ticket/ticket.entity';
...
export class TicketPdfService {
    ...
    async generateTicketPdf(booking: Booking, ticket: Ticket): Promise<string> {
        this.logger.log(
            `Generating PDF for ticket ${ticket.ticket_number} (booking ${booking.pnr_code})`
        );
    ...
    async generateAllTicketsPdf(booking: Booking, tickets: Ticket[]): Promise<string[]> {
        this.logger.log(
            `Generating PDFs for ${tickets.length} tickets (booking ${booking.pnr_code})`
        );
    ...
    private generatePdfContent(
        ...
        const segment = booking.booking_segments?.find(
            (s) =>
                s.booking_passenger.booking_passenger_id ===
        ...
        doc.fontSize(18)
            .font('Helvetica-Bold')
            .text(`Số vé: ${ticket.ticket_number}`, { align: 'center' });
        doc.moveDown(1);

        // Booking Code
        doc.fontSize(14)
            .font('Helvetica')
            .text(`Mã đặt chỗ: ${booking.pnr_code}`, { align: 'center' });
        doc.moveDown(1.5);
        ...
        doc.text(
            `Ngày sinh: ${passenger?.dob ? new Date(passenger.dob).toLocaleDateString('vi-VN') : 'N/A'}`
        );
        doc.text(
            `Giới tính: ${passenger?.gender === 'M' ? 'Nam' : passenger?.gender === 'F' ? 'Nữ' : 'N/A'}`
        );
        ...
        if (schedule && originAirport && destinationAirport) {
            doc.text(
                `Số hiệu chuyến bay: ${schedule.flight_number || flightInstance?.flight_number || 'N/A'}`
            );
            doc.text(
                `Từ: ${originAirport.iata_code || originAirport.name} - ${originAirport.name || ''}`
            );
            doc.text(
                `Đến: ${destinationAirport.iata_code || destinationAirport.name} - ${destinationAirport.name || ''}`
            );
        ...
        doc.fontSize(12).font('Helvetica');
        doc.text(
            `Ngày phát hành: ${ticket.issued_at ? new Date(ticket.issued_at).toLocaleDateString('vi-VN') : 'N/A'}`
        );
        doc.text(`Trạng thái: ${ticket.status === 'active' ? 'Có hiệu lực' : ticket.status}`);
        doc.text(
            `Tổng tiền: ${booking.total_amount?.toLocaleString('vi-VN') || '0'} ${booking.currency?.currency_code || 'VND'}`
        );
        doc.moveDown(1);
