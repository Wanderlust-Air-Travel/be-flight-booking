...
export class BookingNotificationService {
    ...
    constructor(
        ...
        @Optional() private readonly _ticketPdfService: TicketPdfService | null,
    ...
    async sendBookingConfirmation(booking: Booking): Promise<void> {
        ...
        try {
            ...
            this.logger.error(`Failed to send booking confirmation: ${error.message}`, error.stack);
        }
    }
    ...
    private formatFlightDetails(booking: Booking): string {
        ...
        for (const segment of uniqueFlights.values()) {
            ...
            if (route?.origin_airport && route.destination_airport) {
                ...
                const destination =
                    route.destination_airport.iata_code || route.destination_airport.name;
                const flightNumber =
                    schedule?.flight_number || flightInstance.flight_number || 'N/A';
    ...
    async sendTicketConfirmation(booking: Booking, tickets: any[]): Promise<void> {
        ...
        try {
            ...
            if (this.ticketPdfService) {
                try {
                    pdfFilePaths = await this.ticketPdfService.generateAllTicketsPdf(
                        booking,
                        tickets
                    );
                    this.logger.log(
                ...
                this.logger.warn(
                    'TicketPdfService not available. PDF tickets will not be generated.'
                );
            }
    ...
    private formatTicketDetails(booking: Booking, tickets: any[]): any[] {
        ...
        this.logger.log(
            `[formatTicketDetails] Processing ${booking.booking_segments.length} segments with ${tickets.length} tickets (${ticketsWithPassenger} have booking_passenger relation)`
        );
        ...
        for (const segment of booking.booking_segments) {
            ...
            if (!segment.booking_passenger) {
                this.logger.warn(
                    `Segment ${segment.booking_segment_id} does not have booking_passenger relation loaded`
                );
                continue;
            }
            ...
            for (const t of tickets) {
                ...
                if (
                    t.booking_passenger.booking_passenger_id ===
                    segment.booking_passenger.booking_passenger_id
                ) {
                    ticket = t;
                    break;
                }
            ...
            if (!flightInstance) {
                this.logger.warn(
                    `No flight instance found for segment ${segment.booking_segment_id}`
                );
                continue;
            }
