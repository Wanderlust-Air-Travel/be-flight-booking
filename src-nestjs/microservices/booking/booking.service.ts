...
export class BookingService {
    ...
    constructor(
        ...
        private readonly bookingPassengerRepo: Repository<BookingPassenger>,
        ...
        @InjectRepository(BookingSegmentService)
        private readonly bookingSegmentServiceRepo: Repository<BookingSegmentService>,
    ...
    private async generateFareDescriptions(
        ...
        try {
            ...
            this.logger.error(
                `Error fetching fare description rules for ${fareClassCode}/${cabinType}:`,
                error
            );
        ...
        try {
            ...
            for (const segment of dto.segments) {
                ...
                if (!flightInstance) {
                    throw new NotFoundException(
                        `Flight instance ${segment.flightInstanceId} not found`
                    );
                }
                ...
                if (!route) {
                    throw new NotFoundException(
                        `Route not found for flight instance ${segment.flightInstanceId}`
                    );
                }
                ...
                const taxRate =
                    segment.taxAmount !== undefined
                        ? segment.taxAmount / adultBaseFare
                        : pricingInfo.taxRate;
                const feeRate =
                    segment.feeAmount !== undefined
                ...
                const availableSeats = await queryRunner.manager
                    ...
                    .where('seat.flight_instance_id = :instanceId', {
                        instanceId: segment.flightInstanceId,
                    })
            ...
            for (let i = 0; i < passengerDataForValidation.length; i++) {
                ...
                if (p.passengerId && !p.dob) {
                    ...
                    if (passenger) {
                        p.dob =
                            passenger.dob instanceof Date ? passenger.dob : new Date(passenger.dob);
                    }
            ...
            for (const segmentDto of dto.segments) {
                for (let i = 0; i < dto.passengers.length; i++) {
                    ...
                    if (passenger.passengerType === PassengerType.INF && segmentDto.flightSeatId) {
                        throw new BadRequestException(
                            BOOKING_MESSAGES.VALIDATION.INFANT_CANNOT_HAVE_SEAT
                        );
                    }
                }
            }
            ...
            for (const passengerDto of dto.passengers) {
                ...
                if (passengerDto.passengerId) {
                    ...
                    if (!passenger) {
                        throw new NotFoundException(
                            `Passenger ${passengerDto.passengerId} not found`
                        );
                    }
                } else {
                    ...
                    if (
                        passengerDto.passengerType === PassengerType.ADT &&
                        !passengerDto.documentNumber
                    ) {
                        throw new BadRequestException(
                    ...
                    if (Number.isNaN(dobDate.getTime())) {
                        throw new BadRequestException(
                            'Invalid date format for dob. Use YYYY-MM-DD format'
                        );
                    }
                    ...
                    if (existingPassenger) {
                        ...
                        const existingDob =
                            existingPassenger.dob instanceof Date
                        ...
                        const fullnameMatches =
                            existingPassenger.fullname.trim().toLowerCase() ===
                            passengerDto.fullname.trim().toLowerCase();
                        const genderMatches =
                            existingPassenger.gender.toLowerCase() ===
                            passengerDto.gender.toLowerCase();
            ...
            for (
                ...
                passengerIndex < bookingPassengers.length;
                passengerIndex++
            ) {
                ...
                for (
                    let segmentIndex = 0;
                    segmentIndex < validatedSegments.length;
                    segmentIndex++
                ) {
                    ...
                    if (
                        passengerDto.passengerType !== PassengerType.INF &&
                        segmentDto.flightSeatId
                    ) {
                        ...
                        if (!flightSeat) {
                            throw new NotFoundException(
                                `Flight seat ${segmentDto.flightSeatId} not found`
                            );
                        }
                    }
    ...
    async getBookingFareDetails(bookingId: string): Promise<BookingFareDetailsResponseDto> {
        ...
        const fareClassName = this.getFareClassName(
            fareClass.fare_class_code,
            fareClass.description
        );
        const descriptions = await this.generateFareDescriptions(
            fareClass.fare_class_code,
            cabinType
        );
    ...
    async getBooking(bookingId: string, userId: string | null = null): Promise<any> {
        ...
        if (userId !== null) {
            if (booking.user) {
                ...
                if (booking.contact_email) {
                    ...
                    if (
                        user?.email &&
                        user.email.toLowerCase() === booking.contact_email.toLowerCase()
                    ) {
            ...
            if (booking.user) {
                throw new BadRequestException(
                    'This booking belongs to a registered user. Please log in to view it.'
                );
            }
        }
        ...
        const segments = booking.booking_segments.map((segment) => {
            ...
            if (
                ...
                throw new BadRequestException(
                    `Incomplete flight data for segment ${segment.booking_segment_id}`
                );
            }
            ...
            return {
                ...
                fareClass: {
                    ...
                    fareClassName: this.getFareClassName(
        ...
        const passengers = booking.booking_passengers.map((bp) => {
            if (!bp.passenger) {
                this.logger.error(
                    `Passenger is null for booking_passenger ${bp.booking_passenger_id}`
                );
            ...
            if (!bp.passenger.dob) {
                ...
                if (!Number.isNaN(dobDate.getTime())) {
                    ...
                    this.logger.warn(
                        `Invalid dob string for passenger ${bp.passenger.passenger_id}: ${bp.passenger.dob}`
                    );
                ...
                this.logger.warn(
                    `Invalid dob type for passenger ${bp.passenger.passenger_id}: ${typeof bp.passenger.dob}`
                );
                dobString = new Date().toISOString(); // Default to current date
            }
        ...
        try {
            ...
            try {
                ...
                if (error?.code === 'ECONNREFUSED' || error?.message?.includes('ECONNREFUSED')) {
                    throw new BadRequestException(
                        'Reservation service is not available. Please ensure the service is running.'
                    );
                }
                ...
                if (error?.code === 'ETIMEDOUT' || error?.message?.includes('timeout')) {
                    throw new BadRequestException(
                        'Request to reservation service timed out. Please try again.'
                    );
                }
                ...
                if (error?.statusCode === 404 || error?.message?.includes('not found')) {
                    throw new NotFoundException(
                        `Reservation ${reservationId} not found or expired`
                    );
                }
                ...
                // Generic error
                throw new BadRequestException(
                    `Failed to retrieve reservation: ${error?.message || 'Unknown error'}`
                );
            }
            ...
            if (!user) {
                ...
                if (!contactFullname || !contactEmail || !contactPhone) {
                    if (
                        dto.passengers &&
                        dto.passengers.length === 1 &&
                        dto.passengers[0].passengerId
                    ) {
                        const passenger = await queryRunner.manager.findOne(Passenger, {
            ...
            for (const segment of reservation.segments) {
                ...
                const taxRate =
                    segment.taxAmount !== undefined && adultBaseFare > 0
                        ? segment.taxAmount / adultBaseFare
                        : 0;
                const feeRate =
                    segment.feeAmount !== undefined && adultBaseFare > 0
                        ? segment.feeAmount / adultBaseFare
                        : 0;
            ...
            if (totalServicesPrice > 0) {
                this.logger.log(
                    `Total cabin services price added to booking: ${totalServicesPrice} VND`
                );
            }
            ...
            for (const passengerDto of dto.passengers) {
                ...
                if (passengerDto.passengerId) {
                    ...
                    const existingPassenger =
                        user?.user_id && passengerDto.documentNumber
                            ? await queryRunner.manager.findOne(Passenger, {
                                  where: {
                                      document_number: passengerDto.documentNumber,
                                      user: { user_id: user.user_id }, // Use relation, not direct column
            ...
            try {
                ...
                this.logger.log(
                    `Booking confirmation email sent for booking ${savedBooking.booking_id}`
                );
            } catch (error: any) {
                // Log error but don't fail booking creation
                this.logger.error(
                    `Failed to send booking confirmation email: ${error?.message || error}`,
                    error?.stack
                );
            }
            ...
            throw new BadRequestException(
                `Failed to create booking from reservation: ${errorMessage}`
            );
        } finally {
            await queryRunner.release();
        }
        ...
        const ticketItemsRaw = await Promise.all(
            tickets.map(async (ticket) => {
                try {
                    ...
                    const seatsLoaded =
                        segmentsLoaded && segments.some((seg) => seg.flight_seat !== undefined);
                    ...
                    if (!segmentsLoaded || !seatsLoaded) {
                        ...
                        if (reloadedBooking) {
                            ...
                            this.logger.error(
                                `Failed to reload booking ${ticket.booking?.booking_id} for ticket ${ticket.ticket_id}`
                            );
                        }
                    }
                    ...
                    const passengerSegments = segments.filter(
                        (seg) =>
                            seg.booking_passenger?.booking_passenger_id ===
                            ticket.booking_passenger?.booking_passenger_id
                    );
                    ...
                    if (!passengerSegments || passengerSegments.length === 0) {
                        ...
                        this.logger.warn(
                            `Skipping ticket ${ticket.ticket_id} due to missing segment`
                        );
                        return null;
                    }
                    ...
                    const segmentWithSeat = passengerSegments.find(
                        (seg) => seg.flight_seat?.seat_number
                    );
                    const segment =
                        segmentWithSeat ||
                        passengerSegments.sort((a, b) => {
                    ...
                    if (!flightInstance) {
                        this.logger.error(
                            `Flight instance not found for segment ${segment.booking_segment_id}`
                        );
                    ...
                    if (!schedule) {
                        this.logger.error(
                            `Flight schedule not found for flight instance ${flightInstance.flight_instance_id}`
                        );
                    ...
                    if (!route) {
                        this.logger.error(
                            `Route not found for flight schedule ${schedule.flight_schedule_id}`
                        );
                    ...
                    if (!fareClass) {
                        this.logger.error(
                            `Fare class not found for segment ${segment.booking_segment_id}`
                        );
                    ...
                    this.logger.error(
                        `Error transforming ticket ${ticket.ticket_id}: ${error.message}`,
                        error.stack
                    );
    ...
    private calculateRefundAmountForSegments(
        ...
        for (const segment of segments) {
            segmentAmount +=
                Number(segment.base_fare || 0) +
                Number(segment.tax_amount || 0) +
                Number(segment.fee_amount || 0);
        }
    ...
    private calculateRefundAmount(
        ...
        const result = this.calculateRefundAmountForSegments(
            segments,
    ...
    async cancelBooking(
        ...
        try {
            ...
            if (!userId) {
                throw new BadRequestException(
                    'Only authenticated users can cancel bookings. Guest bookings must contact support.'
                );
            }
            ...
            if (booking.booking_segments && booking.booking_segments.length > 0) {
                for (const segment of booking.booking_segments) {
                    ...
                    if (!flightInstance || !fareClass || !route) {
                        this.logger.warn(
                            `Incomplete data for segment ${segment.booking_segment_id}, skipping eligibility check`
                        );
                        continue;
                    }
                    ...
                    if (!cancellationInfo.canCancel) {
                        throw new BadRequestException(
                            cancellationInfo.reason ||
                                'This booking cannot be cancelled due to fare class restrictions or time limits'
                        );
                    }
                }
            }
            ...
            this.logger.log(
                `Booking ${bookingId} cancelled successfully by user ${userId}${refundAmount ? `, refund amount: ${refundAmount} ${booking.currency.currency_code}` : ''}`
            );

            return {
                success: true,
                message: refundAmount
                    ? `Booking cancelled successfully. Refund amount: ${refundAmount.toLocaleString('vi-VN')} ${booking.currency.currency_code}`
    ...
    async cancelTicket(
        ...
        try {
            ...
            if (
                ...
                throw new BadRequestException(
                    `Cannot cancel ticket for booking with status: ${booking.status}`
                );
            }
            ...
            if (!relatedSegment) {
                this.logger.warn(
                    `No segment found for ticket ${ticketId}, proceeding with ticket cancellation only`
                );
            }
            ...
            if (relatedSegment) {
                ...
                if (flightInstance && fareClass && route) {
                    ...
                    if (!cancellationInfo.canCancel) {
                        throw new BadRequestException(
                            cancellationInfo.reason ||
                                'This ticket cannot be cancelled due to fare class restrictions or time limits'
                        );
                    }
                }
            }
            ...
            if (bookingCancelled) {
                ...
                this.logger.log(
                    `All tickets cancelled for booking ${booking.booking_id}, booking automatically cancelled`
                );
            }
    ...
    async getBookingByCode(bookingCode: string): Promise<any> {
        ...
        if (isPnrCode) {
            ...
            if (!uuidRegex.test(bookingCode)) {
                throw new BadRequestException(
                    'Invalid booking code format. Expected PNR code (6 alphanumeric) or booking ID (UUID v7).'
                );
            }
        ...
        const segmentsWithSeats =
            booking.booking_segments?.filter((seg) => seg.flight_seat?.seat_number) || [];
        this.logger.log(
        ...
        for (const seg of booking.booking_segments || []) {
            if (seg.flight_seat) {
                ...
                this.logger.warn(
                    `[getBookingByCode] Segment ${seg.booking_segment_id} does NOT have a seat assigned`
                );
            }
        }
    ...
    async checkInBooking(dto: any): Promise<any> {
        ...
        try {
            ...
            this.logger.log(
                `Check-in validation passed for booking ${dto.bookingCode} (status: ${booking.status}, guest: ${isGuestBooking})`
            );
            ...
            this.logger.log(
                `[checkInBooking] Existing booking has ${existingBooking?.tickets?.length || 0} tickets`
            );
            ...
            if (existingBooking?.tickets && existingBooking.tickets.length > 0) {
                ...
                this.logger.log(
                    `[Re-check-in] Checking for seat changes on booking ${dto.bookingCode}`
                );

                for (const checkInSegment of dto.segments) {
                    const existingSegments = existingBooking.booking_segments.filter(
                        (seg: any) =>
                            seg.flight_instance?.flight_instance_id ===
                            checkInSegment.flightInstanceId
                    );

                    // Filter out INF (infant) passengers - they don't get seats
                    const existingSegmentsNeedingSeats = existingSegments.filter(
                        (seg: any) => seg.booking_passenger?.passenger_type !== 'INF'
                    );
                    this.logger.log(
                        `[Re-check-in] Flight ${checkInSegment.flightInstanceId}: ${existingSegmentsNeedingSeats.length} existing seats, ${checkInSegment.seats.length} new seats`
                    );
                    ...
                    for (let i = 0; i < checkInSegment.seats.length; i++) {
                        ...
                        this.logger.log(
                            `[Re-check-in] Passenger ${i}: old seat ${oldSeatId} vs new seat ${newSeatId}`
                        );

                        if (oldSeatId !== newSeatId) {
                            seatsHaveChanged = true;
                            this.logger.log(
                                `[Re-check-in] ✓ SEAT CHANGE DETECTED: ${oldSeatId} → ${newSeatId}`
                            );
                            break;
                        }
                    }
                ...
                if (!seatsHaveChanged) {
                    ...
                    return {
                        ...
                        message:
                            'Booking has already been checked in. Tickets have already been issued.',
                ...
                const seatAssignments = new Map<
                    string,
                    Array<{ segmentId: string; flightSeatId: string; seatNumber: string }>
                >();
                ...
                for (const checkInSegment of dto.segments) {
                    ...
                    // Note: bookingSegments is from DTO (getBookingByCode), so it has passengerType (camelCase)
                    const segmentsNeedingSeats = bookingSegments.filter(
                        (seg: any) => seg.passengerType !== 'INF'
                    );
                    ...
                    for (let i = 0; i < checkInSegment.seats.length; i++) {
                        ...
                        if (!flightSeat) {
                            throw new NotFoundException(
                                `Flight seat ${seatSelection.flightSeatId} not found`
                            );
                        }
                        ...
                        const bookingSegmentEntity = await queryRunner.manager.findOne(
                            BookingSegment,
                            {
                        ...
                        if (!bookingSegmentEntity) {
                            throw new NotFoundException(
                                `Booking segment ${bookingSegment.segmentId} not found`
                            );
                        }

                        const expectedCabinCode =
                            bookingSegmentEntity.fare_class.cabin_class.cabin_class_code;
                        if (
                            flightSeat.seat_config.cabin_class.cabin_class_code !==
                            expectedCabinCode
                        ) {
                            throw new BadRequestException(
                        ...
                        if (!flightSeat.is_available) {
                            throw new BadRequestException(
                                `Seat ${seatSelection.seatNumber} is not available`
                            );
                        }
                ...
                for (const [segmentId, assignments] of seatAssignments.entries()) {
                    if (assignments.length > 0) {
                        ...
                        if (!bookingSegment) {
                            this.logger.error(
                                `Booking segment ${segmentId} not found when trying to assign seat ${assignment.seatNumber}`
                            );
                        ...
                        if (
                            bookingSegment.flight_seat?.flight_seat_id &&
                            bookingSegment.flight_seat.flight_seat_id !== assignment.flightSeatId
                        ) {
                            this.logger.log(
                                `[Re-check-in] Releasing old seat ${bookingSegment.flight_seat.seat_number} (${bookingSegment.flight_seat.flight_seat_id}) from segment ${segmentId}`
                            );
                            const oldSeat = await queryRunner.manager.findOne(FlightSeat, {
                                where: {
                                    flight_seat_id: bookingSegment.flight_seat.flight_seat_id,
                                },
                            });
                            if (oldSeat) {
                                ...
                                this.logger.log(
                                    `[Re-check-in] Released old seat ${oldSeat.seat_number}`
                                );
                            }
                        }
                        ...
                        if (!flightSeat) {
                            this.logger.error(
                                `Flight seat ${assignment.flightSeatId} not found when trying to assign to segment ${segmentId}`
                            );
                            throw new NotFoundException(
                                `Flight seat ${assignment.flightSeatId} not found`
                            );
                        }
                        ...
                        const savedSegment = await queryRunner.manager.save(
                            BookingSegment,
                            bookingSegment
                        );

                        this.logger.log(
                            `[Re-check-in] Updated booking segment ${segmentId} to seat ${assignment.seatNumber} (${assignment.flightSeatId})`
                        );
                        ...
                        if (!verifiedSegment) {
                            throw new InternalServerErrorException(
                                `Failed to reload booking segment ${segmentId} after seat assignment`
                            );
                        }
                        ...
                        if (
                            !verifiedSegment.flight_seat ||
                            verifiedSegment.flight_seat.flight_seat_id !== assignment.flightSeatId
                        ) {
                            this.logger.error(
                            ...
                            throw new InternalServerErrorException(
                                `Failed to save seat assignment for segment ${segmentId}`
                            );
                        }
                ...
                this.logger.log(
                    `[Re-check-in] Committing seat updates for booking ${dto.bookingCode}...`
                );
                await queryRunner.commitTransaction();
                await queryRunner.release();
                ...
                if (reloadedBooking) {
                    const segmentsWithSeats =
                        reloadedBooking.booking_segments?.filter(
                            (seg) => seg.flight_seat?.seat_number
                        ) || [];
                    this.logger.log(
                ...
                this.logger.log(
                    `[Re-check-in] ✓ Seat update completed and committed for booking ${dto.bookingCode}`
                );

                // CRITICAL: Send updated ticket confirmation email after seat assignment
                this.logger.log(
                    `[Re-check-in] Preparing to send updated ticket confirmation email for booking ${booking.bookingId}...`
                );
                ...
                if (bookingForEmail) {
                    this.logger.log(
                        `[Re-check-in] Booking ${booking.bookingId} loaded successfully. Tickets count: ${existingBooking.tickets.length}, Segments with seats: ${bookingForEmail.booking_segments?.filter((seg) => seg.flight_seat).length || 0}`
                    );
                    ...
                    for (const segment of bookingForEmail.booking_segments || []) {
                        if (segment.flight_seat) {
                            this.logger.log(
                                `[Re-check-in] Segment ${segment.booking_segment_id} has seat ${segment.flight_seat.seat_number} for email`
                            );
                        } else {
                            this.logger.warn(
                                `[Re-check-in] Segment ${segment.booking_segment_id} does NOT have seat for email`
                            );
                        }
                    }
                    ...
                    try {
                        this.logger.log(
                            `[Re-check-in] Calling sendTicketConfirmation for booking ${booking.bookingId}...`
                        );
                        ...
                        this.logger.log(
                            `[Re-check-in] Using ${ticketsForEmail.length} tickets for email. Tickets have booking_passenger: ${ticketsForEmail.filter((t) => t.booking_passenger).length}/${ticketsForEmail.length}`
                        );
                        ...
                        this.logger.log(
                            `[Re-check-in] ✓ Updated ticket confirmation email sent successfully for booking ${booking.bookingId} with seat assignments`
                        );
                    } catch (error: any) {
                        // Log error but don't fail check-in
                        this.logger.error(
                            `[Re-check-in] ✗ Failed to send updated ticket confirmation email: ${error?.message || error}`,
                            error?.stack
                        );
                    }
                } else {
                    this.logger.error(
                        `[Re-check-in] ✗ Booking ${booking.bookingId} not found when trying to send updated ticket confirmation email`
                    );
                }
                ...
                return {
                    ...
                    ticketCount: existingBooking.tickets.length,
                    message:
                        'Seat selection has been updated successfully. Your ticket details have been updated.',
                    alreadyCheckedIn: true,
                };
            }
            ...
            this.logger.log(
                `[checkInBooking] Checking if seats need to be assigned. Existing tickets: ${existingBooking?.tickets?.length || 0}, dto.segments: ${dto.segments?.length || 0}`
            );

            if (!existingBooking?.tickets || existingBooking.tickets.length === 0) {
                this.logger.log(
                    `[Initial check-in] Assigning seats for booking ${dto.bookingCode}...`
                );

                if (!dto.segments || dto.segments.length === 0) {
                    this.logger.error(
                        `[Initial check-in] CRITICAL: No seat selections provided in dto.segments for booking ${dto.bookingCode}. Cannot proceed with seat assignment.`
                    );
                    throw new BadRequestException(
                        'Seat selections are required for check-in. Please select seats for all passengers.'
                    );
                }

                this.logger.log(
                    `[Initial check-in] Processing ${dto.segments.length} flight segments with seat selections`
                );
                ...
                for (const [segmentId, assignments] of seatAssignments.entries()) {
                    if (assignments.length > 0) {
                        ...
                        this.logger.log(
                            `[Initial check-in] Assigned seat ${assignment.seatNumber} (${assignment.flightSeatId}) to booking segment ${segmentId}`
                        );
                ...
                this.logger.log(
                    `[Initial check-in] ✓ Seat assignments completed for booking ${dto.bookingCode}`
                );
            }
            ...
            let tickets: Ticket[] = [];
            if (!existingBooking?.tickets || existingBooking.tickets.length === 0) {
                tickets = await this.createTicketsFromBooking(
                    booking.bookingId,
                    queryRunner.manager
                );
                this.logger.log(
                    `Created ${tickets.length} tickets for booking ${booking.bookingId} during check-in`
                );
            }
            ...
            if (!bookingEntity) {
                this.logger.error(
                    `Booking ${booking.bookingId} not found when trying to send ticket confirmation email`
                );
            } else {
                ...
                // Verify seat information is loaded
                const segmentsWithSeats =
                    bookingBe.booking_segments?.filter((seg) => seg.flight_seat?.seat_number) || [];
                this.logger.log(
                ...
                for (const segment of bookingBe.booking_segments || []) {
                    if (segment.flight_seat) {
                        ...
                        this.logger.warn(
                            `Segment ${segment.booking_segment_id} does NOT have a seat assigned`
                        );
                    }
                }
                ...
                try {
                    ...
                    this.logger.log(
                        `Ticket confirmation email sent for booking ${booking.bookingId}`
                    );
                } catch (error: any) {
                    // Log error but don't fail check-in
                    this.logger.error(
                        `Failed to send ticket confirmation email: ${error?.message || error}`,
                        error?.stack
                    );
                }
            }
            ...
            return {
                ...
                message:
                    'Check-in completed successfully. Tickets have been issued and sent to your email.',
