...
export class PaymentMsController {
    ...
    async handleProcessPayment(
        @Payload() payload: { userId: string | null; dto: CreatePaymentDto }
    ) {
        try {
            this.logger.log(
                `Process payment for booking ${payload.dto.bookingId} (${payload.userId ? 'user' : 'guest'})`
            );
    ...
    async handleGetPayment(@Payload() payload: { userId: string | null; paymentId: string }) {
        try {
            this.logger.log(
                `Get payment ${payload.paymentId} (${payload.userId ? 'user' : 'guest'})`
            );
    ...
    async handleGetPaymentsByBooking(
        @Payload() payload: { userId: string | null; bookingId: string }
    ) {
        try {
            this.logger.log(
                `Get payments for booking ${payload.bookingId} (${payload.userId ? 'user' : 'guest'})`
            );
            const result = await this.paymentService.getPaymentsByBooking(
                payload.userId,
                payload.bookingId
            );
    ...
    async handleUpdatePaymentStatus(
        @Payload() payload: { userId: string; dto: UpdatePaymentStatusDto }
    ) {
        try {
            this.logger.log(
                `Update payment ${payload.dto.paymentId} status to ${payload.dto.status} (${payload.userId === 'system' ? 'webhook' : 'user'})`
            );
