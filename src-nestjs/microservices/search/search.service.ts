...
export class SearchService {
    ...
    async search(dto: SearchFlightsDto) {
        ...
        const outbound = await this.findFlightsForDate(
            route.route_id,
            outboundDate,
            totalPax,
            origin,
        ...
        const inbound = await this.findFlightsForDate(
            inboundRoute.route_id,
            returnDate,
            totalPax,
            destination,
            origin
    ...
    private async findFlightsForDate(
        routeId: string,
        date: Date,
        minSeats: number,
        origin: Airport,
    ...
    async getFareOptions(dto: GetFareOptionsDto): Promise<FareOptionsResponseDto> {
        ...
        const fareOptions: FareOptionDto[] = await Promise.all(
            fareClasses.map(async (fareClass) => {
                ...
                const availableSeats = await this.seatRepo
                    ...
                    .where('seat.flight_instance_id = :instanceId', {
                        instanceId: dto.flightInstanceId,
                    })
                    .andWhere('seat.is_available = :available', { available: true })
                ...
                const displayName = this.getFareClassName(
                ...
                const desc = await this.generateFareDescriptions(
                    fareClass.fare_class_code,
                    dto.cabinType,
                    fareClass.change_rule,
                    fareClass.refund_rule
                );
        ...
        for (const [key, options] of namePriceKeyMap.entries()) {
            if (options.length === 1) {
                ...
                const allSameDescriptions = descriptionsByOption.every(
                    (desc) => desc === descriptionsByOption[0]
                );
    ...
    private async generateFareDescriptions(
        ...
        try {
            ...
            this.logger.error(
                `Error fetching fare description rules for ${fareClassCode}/${cabinTypeString}:`,
                error
            );
    ...
    async getSeatMap(dto: GetSeatMapDto): Promise<SeatMapResponseDto> {
        ...
        allFareClasses.forEach((fareClass) => {
            const cabinType =
                fareClass.cabin_class.cabin_class_code === 'J'
                    ? CabinType.BUSINESS
                    : CabinType.ECONOMY;
    ...
    async getAirports() {
        ...
        return airports.map((airport) => {
            ...
            const citySlug = airport.city
                .toLowerCase()
                .normalize('NFD')
                // biome-ignore lint/suspicious/noMisleadingCharacterClass: intentional Unicode range for Vietnamese diacritics removal
