...
export class SearchFlightsDto {
    ...
    @ApiPropertyOptional({
        description:
            'Return date for round trip in ISO format (YYYY-MM-DD). If provided, tripType will default to round_trip. If not provided or empty, tripType will default to one_way',
        example: '2025-11-24',
    })
    @Transform(({ value }) => {
        // Normalize date format to ensure YYYY-MM-DD with leading zeros
    ...
    @ApiPropertyOptional({
        ...
        description:
            'Type of trip: one_way or round_trip. If not provided, will default to one_way when returnDate is missing, or round_trip when returnDate is provided',
    })
    @IsOptional()
