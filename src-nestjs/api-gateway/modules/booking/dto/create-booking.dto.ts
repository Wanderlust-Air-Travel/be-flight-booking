...
export class CreateBookingPassengerDto {
    @ApiProperty({
        description:
            'Passenger ID (optional - if provided, will use existing passenger; if not, will create new passenger from passengerInfo)',
    ...
    @ApiProperty({
        description:
            'Document number (CCCD/Passport) (required for ADT if passengerId is not provided; optional for CHD and INF)',
...
export class CreateBookingSegmentDto {
    ...
    @ApiProperty({
        description:
            'Base fare amount (optional - will be calculated from fare class if not provided)',
        example: 1448000,
        required: false,
    })
    @IsOptional()
