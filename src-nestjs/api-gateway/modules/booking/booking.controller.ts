...
import type { GetMyTicketsDto } from 'src/microservices/booking/dto/get-my-tickets.dto';
...
export class BookingController {
    ...
    private handleMicroserviceError(error: any, context: string): never {
        ...
        throw new BadRequestException(
            `${COMMON_MESSAGES.ERROR.OPERATION_FAILED}: ${finalErrorMessage}`
        );
    }

    constructor(
        @Inject('BOOKING_CLIENT') private readonly _client: ClientProxy,
        @InjectRepository(User) private readonly _userRepo: Repository<User>,
        @InjectRepository(Passenger) private readonly _passengerRepo: Repository<Passenger>,
    ...
    @ApiQuery({
        ...
        required: true,
        description:
            'Reservation ID (UUID v7) or reservation code (6 alphanumeric). REQUIRED. Booking must be created from a reservation.',
    ...
    async createBooking(
        ...
        try {
            ...
            return await firstValueFrom(
                this.client.send<CreateBookingResponseDto>(
                    BOOKING_MS.PATTERN.CREATE_BOOKING_FROM_RESERVATION,
                    {
                        reservationId,
    ...
    @ApiOperation({
        ...
        description:
            'Get detailed fare information including descriptions and pricing for a specific booking. Public endpoint - no authentication required.',
    })
    @ApiParam({
        name: 'id',
    ...
    async getBookingFareDetails(
        @Param('id') bookingId: string
    ): Promise<BookingFareDetailsResponseDto> {
        try {
            ...
            if (!uuidRegex.test(bookingId)) {
                throw new BadRequestException(
                    BOOKING_MESSAGES.VALIDATION.BOOKING_ID_INVALID_FORMAT
                );
            }

            return await firstValueFrom(
                this.client.send<BookingFareDetailsResponseDto>(
                    BOOKING_MS.PATTERN.GET_FARE_DETAILS,
                    bookingId
                )
            );
        } catch (error: any) {
    ...
    @ApiOperation({
        ...
        description:
            'Get payment-related information including total amount, currency, and contact details for a booking. Public endpoint - no authentication required.',
    })
    @ApiParam({
        name: 'id',
        description: 'Booking ID (UUID v7)',
    ...
    async getBookingPaymentInfo(
        @Param('id') bookingId: string
    ): Promise<BookingPaymentInfoResponseDto> {
        try {
            // Validate UUID v7 format
            const uuidRegex =
                /^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
            ...
            return await firstValueFrom(
                this.client.send<BookingPaymentInfoResponseDto>(
                    BOOKING_MS.PATTERN.GET_PAYMENT_INFO,
                    bookingId
                )
            );
        } catch (error: any) {
    ...
    @ApiOperation({
        ...
        description:
            'Get all tickets booked by the authenticated user with pagination. Returns ticket details including flight information, cancellation eligibility, and booking status. Requires authentication.',
    })
    @ApiQuery({
        name: 'page',
        required: false,
    ...
    @ApiOperation({
        ...
        description:
            'Get all flight journeys (bookings) made by the authenticated user. Returns journey details including origin, destination, flight information, and booking status. Requires authentication.',
    })
    @ApiOkResponse({
    ...
    @ApiOperation({
        ...
        description:
            'Get full booking details including segments, passengers, and flight information. Supports both guest and authenticated bookings. Optional authentication - guest bookings can access their bookings without login.',
    })
    @ApiParam({
        name: 'id',
        description: 'Booking ID (UUID v7)',
    ...
    @ApiOperation({
        ...
        description:
            'Cancel a booking (pending, confirmed, or paid). For paid bookings, OTP verification is required. Only authenticated users can cancel their own bookings. Checks cancellation eligibility based on fare class and time limits. Returns refund amount for paid bookings.',
    })
    @ApiParam({
        name: 'id',
        description: 'Booking ID (UUID v7)',
    ...
    @ApiOkResponse({
        ...
        schema: {
            ...
            properties: {
                ...
                message: {
                    type: 'string',
                    example: 'Booking cancelled successfully. Refund amount: 1,200,000 VND',
                },
                refundAmount: {
                    type: 'number',
                    example: 1200000,
                    description: 'Refund amount (only for paid bookings)',
                },
                cancellationFee: {
                    type: 'number',
                    example: 300000,
                    description: 'Cancellation fee (only for paid bookings)',
                },
            },
        },
    })
    @ApiBadRequestResponse({
        description:
            'Invalid booking ID, booking cannot be cancelled, cancellation deadline passed, or OTP required but not provided/invalid',
    })
    async cancelBooking(
        @Req() req: Request & { user: { userId: string; email: string } },
        @Param('id') bookingId: string,
        ...
        try {
            ...
            if (booking.status === 'paid') {
                ...
                const isOtpVerified = await this.authService.isCancellationOtpVerified(
                    userId,
                    bookingId
                );
                if (!isOtpVerified) {
                    throw new BadRequestException(
                        BOOKING_MESSAGES.VALIDATION.OTP_VERIFICATION_REQUIRED_PAID_BOOKING
                    );
                }
            }
            ...
            const result = await firstValueFrom(
                this.client.send<{
                    ...
                    refundAmount?: number;
                    cancellationFee?: number;
                }>(BOOKING_MS.PATTERN.CANCEL_BOOKING, {
                    bookingId,
                    userId,
                })
            );
            ...
            const finalErrorMessage =
                error?.message || error?.error?.message || errorMessage || 'Unknown error';
    ...
    @ApiOperation({
        ...
        description:
            'Get ticket information including bookingId and bookingStatus. Used for OTP verification flow.',
    })
    @ApiParam({
        name: 'ticketId',
    ...
    async getTicketInfo(
        ...
        try {
            ...
            const result = await firstValueFrom(
                this.client.send<{ ticketId: string; bookingId: string; bookingStatus: string }>(
                    BOOKING_MS.PATTERN.GET_TICKET_INFO,
                    {
                        ticketId,
                        userId,
                    }
    ...
    @ApiOperation({
        ...
        description:
            'Cancel a single ticket from a booking. If all tickets in the booking are cancelled, the booking will be automatically cancelled. For paid bookings, OTP verification is required. Only authenticated users can cancel their own tickets. Returns refund amount for paid bookings.',
    })
    @ApiParam({
        name: 'ticketId',
        description: 'Ticket ID (UUID v7)',
        ...
        schema: {
            ...
            properties: {
                ...
                bookingCancelled: {
                    type: 'boolean',
                    example: false,
                    description: 'Whether the booking was auto-cancelled (all tickets cancelled)',
                },
            },
        },
    })
    @ApiBadRequestResponse({
        description:
            'Invalid ticket ID, ticket cannot be cancelled, cancellation deadline passed, or OTP required but not provided/invalid',
    })
    async cancelTicket(
        @Req() req: Request & { user: { userId: string; email: string } },
        @Param('ticketId') ticketId: string,
        ...
        try {
            ...
            if (ticketInfo.bookingStatus === 'paid') {
                ...
                if (!isOtpVerified) {
                    throw new BadRequestException(
                        BOOKING_MESSAGES.VALIDATION.OTP_VERIFICATION_REQUIRED_PAID_TICKET
                    );
                }
            }
            ...
            if (ticketInfo.bookingStatus === 'paid') {
                await this.authService.deleteCancellationVerificationToken(
                    userId,
                    ticketInfo.bookingId
                );
            }
    ...
    @ApiOperation({
        ...
        description:
            'Get booking details by PNR code (6 alphanumeric) or booking ID (UUID v7). Used for check-in flow. Public endpoint - no authentication required.',
    })
    @ApiParam({
        name: 'code',
    ...
    async getBookingByCode(@Param('code') bookingCode: string): Promise<GetBookingResponseDto> {
        try {
            return await firstValueFrom(
                this.client.send<GetBookingResponseDto>(
                    BOOKING_MS.PATTERN.GET_BOOKING_BY_CODE,
                    bookingCode
                )
            );
        } catch (error: any) {
    ...
    @ApiOperation({
        ...
        description:
            'Check-in a booking by assigning seats and creating tickets. Booking must be paid or confirmed. Seats must match the cabin class selected during booking. Public endpoint - no authentication required.',
    })
    @ApiOkResponse({
