...
import type { WebSocketConnectionInfo } from './types/realtime.types';
...
export class RealtimeGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    constructor(
        ...
        private readonly _seatAvailabilityService: SeatAvailabilityService,
        @Inject(forwardRef(() => ReservationCountdownService))
        private readonly _reservationCountdownService: ReservationCountdownService,
    ...
    async handleConnection(client: Socket) {
        try {
            ...
            const sessionId =
                client.handshake.auth?.sessionId || (client.handshake.query?.sessionId as string);
            ...
            if (token) {
                try {
                    ...
                    authenticated = true;
                    this.logger.log(
                        `Authenticated WebSocket connection: userId=${userId}, socketId=${client.id}`
                    );
                } catch (error) {
                    this.logger.warn(
                        `JWT verification failed for socket ${client.id}: ${error.message}`
                    );
                }
            }
            ...
            if (!authenticated && sessionId) {
                this.logger.log(
                    `Guest WebSocket connection: sessionId=${sessionId}, socketId=${client.id}`
                );
            }
