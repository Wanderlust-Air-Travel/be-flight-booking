...
async function hasExistingSeedData(ds: DataSource): Promise<boolean> {
	...
	if (currencyCount > 0 || fareClassCount > 0) {
		...
		console.log('   Chuyến bay: npm run seed:internal-schedule (VN) hoặc npm run sync:flight-data (Amadeus).');
		return true;
	}
...
async function run() {
	...
	if (!defaultAircraftType) {
		defaultAircraftType = await repos.aircraftType.save(repos.aircraftType.create({
	...
	if (!defaultAircraft) {
		defaultAircraft = await repos.aircraft.save(repos.aircraft.create({
			aircraft_id: uuidv7(),
	...
	if (existingSeatConfig === 0) {
		...
		for (let i = 0; i < seatConfigs.length; i += batchSize) {
			...
			await repos.seatConfig.save(batch.map(sc => repos.seatConfig.create({ ...sc, seat_config_id: uuidv7() })));
		}
	...
	if (!adminUser) {
		adminUser = await repos.user.save(repos.user.create({
			user_id: uuidv7(),
	...
	if (adminRole) {
		const hasAdmin = await repos.userRole.findOne({ where: { user_id: adminUser.user_id, role_code: SystemRole.ADMIN } });
		if (!hasAdmin) {
			await repos.userRole.save(repos.userRole.create({ user_id: adminUser.user_id, role_code: SystemRole.ADMIN }));
			console.log('  Assigned role ADMIN');
		}
	}
	if (customerRole) {
		const hasCustomer = await repos.userRole.findOne({ where: { user_id: adminUser.user_id, role_code: SystemRole.CUSTOMER } });
		if (!hasCustomer) {
			await repos.userRole.save(repos.userRole.create({ user_id: adminUser.user_id, role_code: SystemRole.CUSTOMER }));
			console.log('  Assigned role CUSTOMER');
		}
	}

	console.log('\n--- Reference seed hoàn tất ---');
	console.log('Hãng bay riêng (recommended): npm run seed:internal-schedule  → sân bay VN + lịch nội bộ.');
	console.log('Demo từ Amadeus (optional):   npm run sync:flight-data       → cần AMADEUS_CLIENT_ID/SECRET.');
