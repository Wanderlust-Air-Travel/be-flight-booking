...
async function run() {
	...
	for (const ap of VIETNAM_AIRPORTS) {
		...
		if (!airport) {
			airport = await airportRepo.save(airportRepo.create({
				airport_id: uuidv7(),
				...ap,
			}));
	...
	for (let r = 0; r < routes.length; r++) {
		...
		for (let d = 1; d <= 30; d++) {
			...
			const existing = await instanceRepo.findOne({
				where: { flight_schedule: { flight_schedule_id: schedule.flight_schedule_id }, flight_date: flightDate },
			});
	...
	console.log('  Flight schedules & instances (30 ngày) + seats + giá đã tạo. Trang web tìm chuyến từ DB.');
	await ds.destroy();
}
