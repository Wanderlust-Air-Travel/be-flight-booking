...
async function getAmadeusToken(): Promise<string> {
    ...
    if (!clientId || !clientSecret) {
        throw new Error(
            'Thiếu AMADEUS_CLIENT_ID hoặc AMADEUS_CLIENT_SECRET trong .env. Đăng ký miễn phí tại https://developers.amadeus.com'
        );
    }
...
async function ensureAirport(
    ...
    const created = await airportRepo.save(
        airportRepo.create({
            airport_id: uuidv7(),
            iata_code: code,
            icao_code: null,
...
async function run() {
    ...
    if (!clientId || !clientSecret) {
        ...
        console.log(
            'Đăng ký miễn phí: https://developers.amadeus.com → Create account → My Self-Service APIs → Create new app'
        );
        process.exit(1);
    }
            ...
            try {
                ...
                for (const offer of offers) {
                    ...
                    let route = await routeRepo
                        ...
                        .andWhere('r.destination_airport_id = :did', {
                            did: destAirport.airport_id,
                        })
                        .getOne();
                    if (!route) {
                        ...
                        schedule = await scheduleRepo.save(
                            scheduleRepo.create({
                                ...
                                effective_to: new Date(
                                    depParsed.date.getTime() + 365 * 24 * 60 * 60 * 1000
                                ),
                                status: 'active',
                            })
                        );
                    ...
                    const priceVnd =
                        offer.price.currency === 'EUR'
                            ? Math.round(totalPrice * 27000)
                            : offer.price.currency === 'USD'
                              ? Math.round(totalPrice * 25000)
                    ...
                    await routeFarePriceRepo.save(
                        routeFarePriceRepo.create({
                            ...
                            effective_from: depParsed.date,
                            effective_to: new Date(
                                depParsed.date.getTime() + 30 * 24 * 60 * 60 * 1000
                            ),
                            is_active: true,
                            priority: 0,
                    ...
                    offersProcessed++;
                    if (offersProcessed % 5 === 0)
                        console.log(`  Đã xử lý ${offersProcessed} offers...`);
                }
            } catch (err: unknown) {
