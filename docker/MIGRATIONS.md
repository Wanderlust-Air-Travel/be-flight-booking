...
2. **Chạy migrations tự động** (tạo tables, indexes, triggers)
   ...
   - Không cần file `.env` trong container (env vars từ `docker-compose.yml` và `docker-compose.infrastructure.yml`)
...
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec api-gateway sh
...
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec api-gateway npm run migration:run
```
...
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec api-gateway npm run migration:show

# Hoặc kiểm tra trực tiếp trong database (từ trong container)
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P "Passw0rd123!" -C -d flight_booking_db -Q "SELECT * FROM migrations"
...
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec api-gateway npm run migration:revert
```
...
```bash
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec api-gateway npm run migration:generate -- src/migrations/MigrationName
```
...
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P "Passw0rd123!" -C -d flight_booking_db -Q "SELECT * FROM migrations ORDER BY timestamp DESC"
```
...
docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml logs api-gateway | grep -i migration
```
...
1. **Kiểm tra migrations đã được compile chưa:**
   ```bash
   docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec api-gateway ls -la dist/shared/migrations/
   ```

2. **Kiểm tra TypeORM config:**
   ```bash
   docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml exec api-gateway cat dist/shared/config/typeorm.js
   ```
...
1. **Xem logs chi tiết:**
   ```bash
   docker compose -f docker-compose.yml -f docker-compose.infrastructure.yml logs api-gateway
   ```
